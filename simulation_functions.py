...
def dynamic_prep_and_encoder_assignment(first_sample, k=5, output_size=100, hidden_layer_sizes=[64, 64]):
    """
    Initializes preparation functions and context encoders based on the first sample's datasets. These models are
    shared across all samples.

    Args:
    - first_sample: The first sample in the dataset, used to determine the number and types of models to initialize.
    - k: The number of archetypes, which will be the output size of the context encoders.
    - output_size: The desired output size of the preparation functions.
    - hidden_layer_sizes: The sizes of the hidden layers in MLP preparation functions.

    Returns:
    - A tuple containing two lists: (shared_prep_funcs, shared_context_encoders), where 'shared_prep_funcs'
      are the initialized preparation functions for each dataset type, and 'shared_context_encoders' are the
      corresponding context encoders.
    """
    shared_prep_funcs = []
    shared_context_encoders = []

    for data in first_sample:
        # Assume data might be a batch or a single dataset without a batch dimension
        if data.dim() == 2:  # Likely a single dataset, e.g., a 3x3 matrix
            input_shape = data.numel()  # Total number of elements in the matrix
            prep_func = MLPPreparationFunction(input_shape=input_shape, output_size=output_size, hidden_layer_sizes=hidden_layer_sizes)
        elif data.dim() > 2:  # True batch dimension
            input_shape = int(torch.prod(torch.tensor(data.shape[1:])).item())  # Product of dimensions except the batch
            prep_func = MLPPreparationFunction(input_shape=input_shape, output_size=output_size, hidden_layer_sizes=hidden_layer_sizes)
        else:  # Linear case
...
class ArchetypeDictionary(nn.Module):
    def __init__(self, num_genes, num_archetypes):
        """
        ...
        self.archetypes = nn.Parameter(torch.randn(num_archetypes, num_genes, num_genes+2))
