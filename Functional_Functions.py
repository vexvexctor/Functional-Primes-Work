...
class MLPPreparationFunction(PreparationFunction):
    def __init__(self, input_shape, output_size, hidden_layer_sizes=[64, 64]):
        super().__init__(input_shape, output_size)
        layers = []
        # Adjusting input_shape if necessary
    ...
    def forward(self, x):
        #print(f"Input shape before MLP processing: {x.shape}")  # Debug print
        if x.dim() > 2:
            # Ensure the data is flattened if it's multidimensional
            x = x.view(x.size(0), -1)
...
def dynamic_prep_and_encoder_assignment(first_sample, k=5, output_size=100, hidden_layer_sizes=[64, 64]):
    ...
    for data in first_sample:
        if data.dim() > 2:  # Use MLP for multidimensional data
            input_shape = int(torch.prod(torch.tensor(data.shape[1:])).item())  # Calculate total features for MLP
            prep_func = MLPPreparationFunction(input_shape=input_shape, output_size=output_size, hidden_layer_sizes=hidden_layer_sizes)
        else:  # Use Linear for 1D data
...
class ArchetypeDictionary(nn.Module):
    def __init__(self, num_genes, num_archetypes):
        ...
        self.archetypes = nn.Parameter(torch.randn(num_archetypes, num_genes, num_genes))
